"""Batch OCR over MAPAS JPGs using extract_year_and_datum (multiprocess version).

This script is similar to `run_fbds_ocr_batch.py` but uses a process
pool to run OCR on multiple images in parallel, using all available
...
def main() -> None:
    ...
    output_csv = Path("fbds_mapas_ocr_mp.csv").resolve()

    print(f"[MP] Scanning MAPAS JPEGs under: {download_root}")
    run_batch_mp(download_root=download_root, output_csv=output_csv)
